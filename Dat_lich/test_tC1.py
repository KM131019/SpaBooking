...
class TestTC1():
  ...
  def test_tC1(self):
    ...
    time.sleep(3)
    # Chọn khung giờ
    self.driver.find_element(By.XPATH, "//button[contains(.,'17:30')]").click()
    time.sleep(3)
    # Điền thông tin
    self.driver.find_element(By.CSS_SELECTOR, "input[id*='full-name']").clear()
    self.driver.find_element(By.CSS_SELECTOR, "input[id*='full-name']").send_keys("tran my")
    ...
    self.driver.find_element(By.CSS_SELECTOR, "textarea[id*='notes']").send_keys("dfghjk8415")
    self.driver.find_element(By.CSS_SELECTOR, "button[title='Book now']").click()
    time.sleep(3)
    elements = self.driver.find_elements(By.CSS_SELECTOR, ".bookly-done-step-secondary-text-mark")
